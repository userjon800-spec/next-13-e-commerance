...
const ShoppingCard = () => {
  ...
  let removeProduct = (id: number) => {
    let updateCard = products.filter((pro) => pro.id !== id);
    localStorage.setItem("cart", JSON.stringify(updateCard));
    setProducts(updateCard);
  };
  const [total, setTotal] = useState<number>(0);
  let handleIncrement = (id: number) => {
    let updatedCart = products.map((pro) => {
      if (pro.id === id) {
  ...
  let handleDecrement = (id: number) => {
    let exisProducts = products.find((pro) => pro.id === id);
    if (exisProducts?.quantity === 1) {
      removeProduct(exisProducts.id);
    } else {
      let updatedCart = products.map((pro) => {
        if (pro.id === id) {
          return { ...pro, quantity: pro.quantity - 1 };
  ...
  useEffect(() => {
    let totals = products.reduce((acc, pro) => {
        ...
        <div className="flex h-[calc(100vh-80px)] items-center justify-center p-5 bg-white w-full">
          <div className="text-center">
            ...
            <p className="text-slate-600 mt-5 lg:text-lg">
              The page you are looking for doesn&rsquo;t exist or <br />
              has been removed.
            </p>
            <Link href={"/products"}>
